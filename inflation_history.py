...
# ==========================================
# 2. HISTORICAL ANALYSIS: VOLATILITY
# ==========================================
print("Analyzing Historical Volatility and Structural Breaks...")

# 1. Calculate Yearly Average
yearly_data = df_long['Inflation_Rate'].resample('YE').mean()

# 2. Calculate Rolling Volatility
rolling_volatility = df_long['Inflation_Rate'].rolling(window=12).std()

# 3. Identify High Volatility Periods
high_vol_threshold = 1.5
high_vol_years = rolling_volatility[rolling_volatility > high_vol_threshold]

print(f"\nIdentified {len(high_vol_years)} months of extreme volatility (Std Dev > {high_vol_threshold}).")
...
# Plot 1: Inflation Rate (Left Axis)
color = 'tab:blue'
ax1.set_xlabel('Year')
ax1.set_ylabel('Inflation Rate (%)', color=color, fontsize=12)
ax1.plot(df_long.index, df_long['Inflation_Rate'], color=color, alpha=0.3, label='Monthly Rate')
ax1.plot(yearly_data.index, yearly_data, color='blue', linewidth=2, label='Yearly Average')
ax1.tick_params(axis='y', labelcolor=color)

# Plot 2: Volatility (Right Axis)
ax2 = ax1.twinx()  
color = 'tab:red'
ax2.set_ylabel('Volatility (12-Month Rolling Std Dev)', color=color, fontsize=12)
ax2.plot(rolling_volatility.index, rolling_volatility, color=color, linewidth=1.5, linestyle='--', label='Volatility')
ax2.tick_params(axis='y', labelcolor=color)

# Fill high volatility areas
ax2.fill_between(rolling_volatility.index, 0, rolling_volatility, 
                 where=(rolling_volatility > high_vol_threshold), 
                 color='red', alpha=0.1, label='High Volatility Era')

# Add Historical Annotations
events = {
    '1985-06-01': "Currency Crisis",
    '1994-04-01': "First Democratic Election",
    '2008-09-01': "Global Financial Crisis",
    '2020-03-01': "COVID-19 Pandemic"
}

# --- FIX: Added .items() here ---
for date_str, label in events.items():
    date = pd.to_datetime(date_str)
    ax1.axvline(x=date, color='black', linestyle=':', linewidth=1)
    ax1.text(date, ax1.get_ylim()[1] - 1, label, rotation=90, verticalalignment='top', fontsize=9, fontweight='bold')

plt.title('South Africa Inflation: 50 Years of Stability vs. Chaos', fontsize=16)
fig.tight_layout()
plt.savefig('inflation_volatility_history.png', dpi=300)
plt.show()

print("\n✅ Historical Analysis Complete. Check 'inflation_volatility_history.png'.")